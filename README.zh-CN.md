...
Ainotation 采用[开发与非商业使用许可证](./LICENSE)，属于**源码可见软件**。

个人和公司的内部开发、调试可免费使用，包括开发商业应用。商业分发、托管服务，以及将 Ainotation 集成到生产产品中，须另行获得 [nightire](https://github.com/nightire) 的书面授权。不包含 Ainotation 的应用，以及用户创建的反馈和截图，不会仅因使用了该工具而受到本许可证限制。

这不是 OSI 认可的开源许可证，完整条款以 [LICENSE](./LICENSE) 为准。
